...
export default class SociSidebar extends SociComponent {
  ...
  _checkCommunityChange() {
    ...
    if(this._voice?._voiceRoom && this._voice._voiceCommunity !== community) this._voiceDisconnect()
  ...
  logout(){
    ...
    if (this._voice) this._voice._voicePresenceByChannel = {}
    this._renderVoicePresenceParticipants()
  ...
  // --- Voice (LiveKit) — lazy-loaded SociVoice controller (components/voice/) ---
  _voice = null
  _voiceLoad = null

  _ensureVoice() {
    if (!this._voice && !this._voiceLoad) {
      this._voiceLoad = import('./voice/soci-voice.js')
        .then(({ default: SociVoice }) => { this._voice = new SociVoice(this) })
    }
    return this._voiceLoad.then(() => this._voice)
  }

  joinVoiceChannel(channel) {
    return this._ensureVoice().then(v => v.joinVoiceChannel(channel))
  }

  disconnectVoice() {
    return this._ensureVoice().then(v => v.disconnectVoice())
  }

  _voiceDisconnect() {
    return this._ensureVoice().then(v => v._voiceDisconnect())
  }

  _startVoicePresenceSocket(trigger) {
    if (!this.authToken) return
    this._ensureVoice()
      .then(v => v._startVoicePresenceSocket(trigger))
      .catch(err => console.warn('[VoicePresenceWS] lazy load failed', err))
  }

  _stopVoicePresenceSocket(reason) {
    this._voice?._stopVoicePresenceSocket(reason)
  }

  _updateVoiceUI() {
    this._voice?._updateVoiceUI()
  }

  _renderVoicePresenceParticipants() {
    this._voice?._renderVoicePresenceParticipants()
  }
}